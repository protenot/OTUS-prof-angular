...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (savedLanguage) {
      ...
      this.selectedLearningLanguage =
        this.settingService.getSelectedLearningLanguage();
    }
  }
  ...
  translateToEnglish(word: string) {
    ...
    this.translatorService
      .translateRussianWord(this.wordToTranslate)
  ...
  translateToRussian(word: string) {
    this.wordToTranslate = word;
    this.translatorService
      .translateWord(this.wordToTranslate, 'russian')
      .subscribe((translatedText) => {
        this.translatedWord = translatedText.toLowerCase();
        console.log('слово', word, 'перевод', this.translatedWord);
  ...
  addToDictionary() {
    ...
    const wordsFromLS = this.dictionaryService.getWordsByLanguage(
      this.selectedLearningLanguage,
    );
